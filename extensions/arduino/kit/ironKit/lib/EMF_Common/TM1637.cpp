...
// Print API ///////////////////////////////////////////////////////////////////
// single byte
size_t  TM1637::write(uint8_t byte) {
...
size_t  TM1637::write(const char* str) {
...
size_t  TM1637::write(const uint8_t* buffer, size_t size) {
...
// Liquid cristal API
void TM1637::begin(uint8_t cols, uint8_t rows) {
  _numCols = cols;
  _numRows = rows;
  clear();
};

void TM1637::init(uint8_t cols, uint8_t rows) {
  begin(cols, rows);
}
...
void TM1637::home(void) {
  setCursor(0, 0);
};

void TM1637::setCursor(uint8_t row, uint8_t col) {
...
// TM1637 public methods
void  TM1637::blink(uint8_t blinkDelay, uint8_t repeats, uint8_t maxBacklight, uint8_t minBacklight) {
...
void  TM1637::setPrintDelay(uint16_t printDelay) {
  _printDelay = printDelay;
};

bool  TM1637::getColonOn(void) {
  return (_colonOn);
};

void  TM1637::setColonOn(bool setToOn) {
  _colonOn = setToOn;
}
...
size_t  TM1637::encode(uint8_t* buffer, const char* str, size_t bufferSize) {
  size_t i;
...
size_t  TM1637::encode(uint8_t* buffer, const uint8_t* byteArr, size_t bufferSize) {
  size_t i;
...
bool    TM1637::command(uint8_t pinClk, uint8_t pinDIO, uint8_t cmd) {
...
bool    TM1637::command(uint8_t pinClk, uint8_t pinDIO, const uint8_t* commands, uint8_t length) {
...
uint8_t TM1637::comReadByte(void) const {
  uint8_t readKey = 0;
...
void    TM1637::comWriteByte(uint8_t pinClk, uint8_t pinDIO, uint8_t command) {
  // CLK in bits
  for ( uint8_t i = 0; i < 8; i++) {
...
void    TM1637::comStart(void) const {
  comStart(_pinClk, _pinDIO);
};

void    TM1637::comStart(uint8_t pinClk, uint8_t pinDIO) {
  digitalHigh(pinDIO);   // DIO HIGH
  digitalHigh(pinClk);   // CLK HIGH
  delayMicroseconds(TM1637_CLK_DELAY_US);
...
void    TM1637::comStop(uint8_t pinClk, uint8_t pinDIO) {
  digitalLow(pinClk);   // CLK LOW
  delayMicroseconds(TM1637_CLK_DELAY_US);
...
bool    TM1637::comAck(uint8_t pinClk, uint8_t pinDIO) {
