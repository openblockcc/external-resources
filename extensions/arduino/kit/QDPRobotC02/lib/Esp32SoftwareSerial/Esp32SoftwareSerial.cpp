...
Esp32SoftwareSerial.cpp - Implementation of the Arduino software serial for ESP8266/ESP32.
...
Esp32SoftwareSerial::Esp32SoftwareSerial(int8_t rxPin, int8_t txPin, bool invert)
{
    m_isrOverflow = false;
    m_rxPin = rxPin;
    m_txPin = txPin;
...
bool Esp32SoftwareSerial::isValidGPIOpin(int8_t pin) {
#if defined(ESP8266)
...
void Esp32SoftwareSerial::begin(uint32_t baud, Esp32SoftwareSerialConfig config,
    ...
    m_parityMode = static_cast<Esp32SoftwareSerialParity>(config & 070);
...
uint32_t Esp32SoftwareSerial::baudRate() {
    return ESP.getCpuFreqMHz() * 1000000UL / m_bitCycles;
}

void Esp32SoftwareSerial::setTransmitEnablePin(int8_t txEnablePin) {
...
void Esp32SoftwareSerial::enableIntTx(bool on) {
    m_intTxEnabled = on;
}

void Esp32SoftwareSerial::enableTx(bool on) {
...
size_t Esp32SoftwareSerial::read(uint8_t * buffer, size_t size) {
...
size_t Esp32SoftwareSerial::readBytes(uint8_t * buffer, size_t size) {
...
void ICACHE_RAM_ATTR Esp32SoftwareSerial::preciseDelay(bool sync) {
    if (!sync)
    {
...
void ICACHE_RAM_ATTR Esp32SoftwareSerial::writePeriod(
...
size_t Esp32SoftwareSerial::write(uint8_t byte) {
    return write(&byte, 1);
}

size_t Esp32SoftwareSerial::write(uint8_t byte, Esp32SoftwareSerialParity parity) {
    return write(&byte, 1, parity);
}

size_t Esp32SoftwareSerial::write(const uint8_t * buffer, size_t size) {
    return write(buffer, size, m_parityMode);
}

size_t ICACHE_RAM_ATTR Esp32SoftwareSerial::write(const uint8_t * buffer, size_t size, Esp32SoftwareSerialParity parity) {
    if (m_rxValid) { rxBits(); }
...
void Esp32SoftwareSerial::rxBits() {
...
void Esp32SoftwareSerial::rxBits(const uint32_t & isrCycle) {
...
void ICACHE_RAM_ATTR Esp32SoftwareSerial::rxBitISR(Esp32SoftwareSerial * self) {
...
void ICACHE_RAM_ATTR Esp32SoftwareSerial::rxBitSyncISR(Esp32SoftwareSerial * self) {
...
void Esp32SoftwareSerial::onReceive(Delegate<void(int available), void*> handler) {
    receiveHandler = handler;
}

void Esp32SoftwareSerial::perform_work() {
    if (!m_rxValid) { return; }
    rxBits();
    if (receiveHandler) {
        int avail = m_buffer->available();
