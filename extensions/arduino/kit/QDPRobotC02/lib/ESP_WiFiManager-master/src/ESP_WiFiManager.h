...
const char HTTP_HEAD_START[] PROGMEM = "<!DOCTYPE html><html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1, user-scalable=no\"/><title>{v}</title>";
...
const char HTTP_SCRIPT_NTP_MSG[] PROGMEM = "<p>您的时区是 : <b><label id='timezone'></b><script>document.getElementById('timezone').innerHTML = timezone.name();</script></p>";

#if 1
const char HTTP_SCRIPT_NTP[] PROGMEM = "<script src='https://cdnjs.cloudflare.com/ajax/libs/jstimezonedetect/1.0.4/jstz.min.js'></script><script>var timezone=jstz.determine();console.log('您的时区是:' + timezone.name());document.getElementById('timezone').innerHTML = timezone.name();</script>";
#else
...
const char HTTP_PORTAL_OPTIONS[] PROGMEM = "<form action=\"/wifi\" method=\"get\"><button class=\"btn\">配置网络</button></form><br/><form action=\"/i\" method=\"get\"><button class=\"btn\">硬件信息</button></form><br/><form action=\"/close\" method=\"get\"><button class=\"btn\">退出</button></form><br/>";
...
const char HTTP_SAVED[] PROGMEM = "<div class=\"msg\"><b>WiFi信息已更改</b><br>尝试连接到网络<b>{x}</b>。<br>请10秒钟左右检查连接。<a href=\"/\">主页</a> <p/>当前热点{v}将连接到<b>{x}</b>网络并尝试重启。 若软件重启失败你可能需要手动重启。</div>";
