...
#ifndef __Esp32SoftwareSerial_h
#define __Esp32SoftwareSerial_h

#include "circular_queue/circular_queue.h"
#include <Stream.h>

enum Esp32SoftwareSerialParity : uint8_t {
...
enum Esp32SoftwareSerialConfig {
...
public:
    ...
    Esp32SoftwareSerial(int8_t rxPin, int8_t txPin = -1, bool invert = false);
    Esp32SoftwareSerial(const Esp32SoftwareSerial&) = delete;
    Esp32SoftwareSerial& operator= (const Esp32SoftwareSerial&) = delete;
    virtual ~Esp32SoftwareSerial();
    /// Configure the Esp32SoftwareSerial object for use.
    ...
    void begin(uint32_t baud, Esp32SoftwareSerialConfig config,
    ...
    void begin(uint32_t baud, Esp32SoftwareSerialConfig config = SWSERIAL_8N1) {
    ...
    size_t write(uint8_t byte, Esp32SoftwareSerialParity parity);
    size_t write(const uint8_t* buffer, size_t size) override;
    size_t write(const char* buffer, size_t size) {
        return write(reinterpret_cast<const uint8_t*>(buffer), size);
    }
    size_t write(const uint8_t* buffer, size_t size, Esp32SoftwareSerialParity parity);
    size_t write(const char* buffer, size_t size, Esp32SoftwareSerialParity parity) {
...
private:
    ...
    static void rxBitISR(Esp32SoftwareSerial* self);
    static void rxBitSyncISR(Esp32SoftwareSerial* self);
    ...
    uint8_t m_pduBits;
    bool m_intTxEnabled;
    Esp32SoftwareSerialParity m_parityMode;
    uint8_t m_stopBits;
