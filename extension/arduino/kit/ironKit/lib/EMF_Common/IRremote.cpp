...
void IRremote::sendNEC(uint16_t KeyValue) {
  send38KHZ(280, 1);//Send 9ms start code
  send38KHZ(140, 0);//Send a 4.5ms result code
  sendcode(ADD);//User ID
  dat = ~ADD;
  sendcode(dat);//User ID reverse code
  sendcode(KeyValue);//Operation code
  dat = ~KeyValue;
  sendcode(dat);//Opcode inversion
  send38KHZ(21, 1);//Send end code
}

void IRremote::send38KHZ(int x, int y) {
  for (int i = 0; i < x; i++) { //15=386US
    if ( y == 1) {
      digitalWrite(irparams.recvpin, 1);
      delayMicroseconds(9);
      digitalWrite(irparams.recvpin, 0);
      delayMicroseconds(9);
    }
    else {
      digitalWrite(irparams.recvpin, 0);
...
void IRremote::sendcode(uint8_t x) {
  for (int i = 0; i < 8; i++) {
    if ( (x & 0x01) == 0x01) {
      send38KHZ(23, 1);
      send38KHZ(64, 0);
